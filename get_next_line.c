...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/21 21:59:48 by azane             #+#    #+#             */
/*   Updated: 2022/01/05 03:08:52 by azane            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_after(char *buf)
{
	...
	k++;
	str = (char *)malloc(sizeof(char) * (ft_strlen(buf) - k + 1));
	if (!str)
		return (NULL);
	while (buf[k])
		str[i++] = buf[k++];
...
char	*get_next_line(int fd)
{
	...
	if (!buf)
		return (NULL);
	save_line = gnl(fd, buf, save_line);
	if (!save_line)
		return (NULL);
	result = get_before(save_line);
	save_line = get_after(save_line);
